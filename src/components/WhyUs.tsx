    ...
    <section className="mt-10 md:mt-20">
      <div>
        <div className="text-center mb-2">
          <span
            ref={addToRefs}
            className="inline-block px-4 md:px-20 py-2 font-medium text-white border border-white border-opacity-20 rounded-full text-sm"
          >
            اصقل
          </span>
        </div>
        <div className="text-center mb-8 px-4">
          <h2
            ref={addToRefs}
            className="text-white text-3xl mt-4 mb-2 md:text-[48px] font-bold"
          >
            ايش الفرق
          </h2>
          <p ref={addToRefs} className="text-gray-300 pt-3">
          ...
          {features.map((feature, index) => (
            <AccordionItem key={index} value={`item-${index}`}>
              <AccordionTrigger className="mt-10">
                <h2
                  ref={addToRefs}
                  className="text-2xl md:text-[40px] font-bold text-right"
                >
                  {feature.title}
                </h2>
              </AccordionTrigger>
