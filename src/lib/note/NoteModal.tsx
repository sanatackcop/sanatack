...
import { createNewNoteApi } from "@/utils/_apis/learnPlayground-api";
import {
  getErrorMessage,
...
export default function NoteModal({
  ...
  const handleCreate = useCallback(async () => {
    ...
    setGenerating(true);
    try {
      await createNewNoteApi({ id: workspaceId, language });
      toast.success(t("note.modal.created", "Note created"));
      onClose(true);
    } catch (err) {
  ...
  }, [disabled, workspaceId, language, t, onClose, isRTL]);
  ...
  return (
    <Modal
      ...
      title={t("note.modal.title", "Create Note")}
      description={t(
        "note.modal.description",
        "Select specific concepts and customize your Note"
      )}
      variant="default"
      confirmLabel={
        generating ? (
          <span className="inline-flex items-center gap-2">
            ...
            {t("note.modal.generating", "Generating…")}
          </span>
        ) : (
          <span className="inline-flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="language-select">
            {t("note.modal.language.label", "Language")}
          </Label>
          <Select
            value={language}
          ...
          >
            <SelectTrigger id="language-select" dir={direction}>
              <SelectValue
                placeholder={t(
                  "note.modal.language.placeholder",
                  "Select language"
                )}
              />
            </SelectTrigger>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="inline-flex items-center gap-2 text-xs text-muted-foreground">
                ...
                {t("note.modal.hint", "Fill the form to enable generation")}
              </span>
            </TooltipTrigger>
            <TooltipContent dir={direction}>
              {t(
                "note.modal.hintDetail",
